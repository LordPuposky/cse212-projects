using System;

public static class ArraySelector
{
    public static void Run()
    {
        var l1 = new[] { 1, 2, 3, 4, 5 };
        var l2 = new[] { 2, 4, 6, 8, 10 };
        var select = new[] { 1, 1, 1, 2, 2, 1, 2, 2, 2, 1 };
        var intResult = ListSelector(l1, l2, select);
        Console.WriteLine("<int[]>{" + string.Join(", ", intResult) + "}"); // Expected: <int[]>{1, 2, 3, 2, 4, 4, 6, 8, 10, 5}
    }

    public static int[] ListSelector(int[] list1, int[] list2, int[] select)
    {
        // Create the result array based on the selector's length
        var result = new int[select.Length];

        // Pointers to track current position in each source list
        int index1 = 0;
        int index2 = 0;

        // Iterate through the selector to pick the correct value
        for (int i = 0; i < select.Length; i++)
        {
            if (select[i] == 1)
            {
                result[i] = list1[index1];
                index1++;
            }
            else
            {
                result[i] = list2[index2];
                index2++;
            }
        }

        return result;
    }
}